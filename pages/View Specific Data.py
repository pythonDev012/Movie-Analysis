...
with rating:
    st.markdown("<p style = 'font-size: 20px;'>"+str(movies_df[movies_df['Title'] == name]['Rating'].tolist()[0])+"/10"+"</p>", unsafe_allow_html = True)
with vote_count:
...
if movies_df[movies_df['Title'] == name]['Revenue'].tolist()[0] != 0:
    st.markdown("<p style = 'font-size: 18px'>USD " + str(movies_df[movies_df['Title'] == name]['Revenue'].tolist()[0])+'</p>', unsafe_allow_html = True)
else:
